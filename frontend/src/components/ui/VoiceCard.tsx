...
export function VoiceCard({ profile, onClick, onInspectVector, viewMode }: VoiceCardProps) {
  ...
  const togglePlay = (e: React.MouseEvent) => {
    ...
    // In a real app, this would trigger audio playback of the reference sample
  };
    ...
    return (
      ...
      >
        ...
        <div className="flex items-center gap-6">
          ...
          <button 
            onClick={togglePlay}
            className="w-8 h-8 flex items-center justify-center rounded-full bg-indigo-600/20 text-indigo-400 hover:bg-indigo-600 hover:text-white transition-colors"
          >
            {isPlaying ? <Pause size={14} fill="currentColor" /> : <Play size={14} fill="currentColor" className="ml-0.5" />}
          </button>
          
          <button onClick={handleInspect} className="text-slate-500 hover:text-slate-300">
  ...
  return (
    <div 
      onClick={() => onClick(profile)}
      className="flex flex-col p-5 glass-panel rounded-2xl hover:bg-slate-800/80 cursor-pointer transition-all border border-slate-700/50 group relative overflow-hidden"
    >
      <div className="absolute top-4 right-4 flex gap-2">
        ...
        <button onClick={handleInspect} className="w-6 h-6 flex items-center justify-center rounded-full bg-slate-800 text-slate-400 hover:text-white hover:bg-slate-700 z-10 transition-colors">
      ...
      <div className="flex flex-col items-center mb-4 mt-2">
        <div className="relative">
          ...
          <button 
            onClick={togglePlay}
            className="absolute -bottom-2 -right-2 w-8 h-8 flex items-center justify-center rounded-full bg-indigo-600 text-white shadow-lg hover:scale-110 transition-transform"
          >
            {isPlaying ? <Pause size={14} fill="currentColor" /> : <Play size={14} fill="currentColor" className="ml-0.5" />}
          </button>
        </div>
      ...
      <div className="w-full h-8 flex items-center justify-center gap-[2px] opacity-40 group-hover:opacity-100 transition-opacity mt-2">
        {Array.from({ length: 24 }).map((_, i) => (
          <div key={i} className="w-1 bg-indigo-400 rounded-full" style={{ height: `${Math.max(20, Math.random() * 100)}%` }} />
        ))}
      </div>
